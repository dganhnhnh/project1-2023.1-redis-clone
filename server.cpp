...
//-- nếu hợp lệ, đẩy req str vào 1 cái vector chứa từng phần
...
static void out_err(std::string &out, int32_t code, const std::string &msg) {
    ...
    //-- why have to reference then de-reference when type casting
...
//-- truyền con trỏ hàm vào arg 2 
static void ht_scan(HTab *tab, void (*f)(HNode *, void *), void *arg) {
    if (tab->size == 0) {
        return;
    }
...
static void do_keys(std::vector<std::string> &cmd, std::string &out) {
    ...
    ht_scan(&g_data.db.ht1, &cb_scan, &out);
    ht_scan(&g_data.db.ht2, &cb_scan, &out);
}
